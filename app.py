...
from pdf_generator import dataframe_to_pdf_bytes
...
def main() -> None:
    ...
    st.title(APP_TITLE)
    st.write(
        "Upload one GMAO STS Issue Excel export at a time, confirm its Requisition Number, "
...
- The export does not contain the Requisition Number, so the app prompts for it per file.
...
- For aisles above `10`, vertical `A` is ground-level and `B+` is height/FLT.
    ...
    separate_issue_line_threshold = settings_col1.number_input(
        "Separate issue pack threshold",
        min_value=1,
        max_value=500,
        value=20,
        step=1,
    ...
    master_pick_page_line_limit = settings_col2.number_input(
        "Master pick page line limit",
        min_value=1,
        max_value=100,
        value=20,
        step=1,
    ...
    if uploaded_file is not None:
        ...
        st.info("Upload one STS Issue export, then enter and confirm its Requisition Number.")
    ...
    selected_pack = select_pick_pack(pick_packs)
    pick_sheets = selected_pack["pick_sheets"]
    tabs = st.tabs(["All lines", "Ground-level", "Height/FLT"])
    with tabs[0]:
        st.dataframe(pick_sheets["all"], use_container_width=True, hide_index=True)
    with tabs[1]:
        st.dataframe(pick_sheets["ground"], use_container_width=True, hide_index=True)
    with tabs[2]:
        st.dataframe(pick_sheets["height"], use_container_width=True, hide_index=True)
    ...
    for pack in pick_packs:
        with st.expander(pack["name"], expanded=len(pick_packs) == 1):
            ...
            ground_pdf = dataframe_to_pdf_bytes(
                pack["pick_sheets"]["ground"],
                f"{pack['name']} - Ground Level",
            ...
            height_pdf = dataframe_to_pdf_bytes(
                pack["pick_sheets"]["height"],
                f"{pack['name']} - Height/FLT",
                released_by=release_metadata.get("released_by", ""),
                released_at=release_metadata.get("released_at", ""),
                master_pick_page_line_limit=active_master_pick_page_line_limit,
                requisition_zone_map=zone_map,
            ...
            col2.download_button(
                "Download ground PDF",
                data=ground_pdf,
                file_name=f"caf_{pack['slug']}_ground_level_pick_sheet.pdf",
            ...
            col3.download_button(
                "Download height PDF",
                data=height_pdf,
                file_name=f"caf_{pack['slug']}_height_flt_pick_sheet.pdf",
                mime="application/pdf",
                use_container_width=True,
            )
...
def select_pick_pack(pick_packs: list[dict]) -> dict:
    pack_names = [pack["name"] for pack in pick_packs]
    selected_name = st.selectbox("Picking pack", pack_names)
    return next(pack for pack in pick_packs if pack["name"] == selected_name)
...
def requisition_number_dialog() -> None:
    ...
    if col1.button("Cancel", use_container_width=True):
        ...
        st.rerun()
    if col2.button("Confirm Requisition Number", type="primary", use_container_width=True):
