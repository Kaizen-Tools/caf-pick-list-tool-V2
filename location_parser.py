...
def parse_location(value: Any) -> ParsedLocation:
    ...
    if not match:
        ...
        if _is_special_text_location(raw):
            return ParsedLocation(raw, None, None, None, None, "Ground-level", "Special non-numeric location")
    ...
    pick_zone = classify_location(aisle=aisle, bay=bay, vertical=vertical)
...
def classify_location(aisle: int | None, vertical: str | None, bay: int | None = None) -> str:
    ...
    if aisle == 65 and bay is not None and 1 <= bay <= 6:
        return "Ground-level"
...
def _is_special_text_location(raw: str) -> bool:
    return any(char.isalpha() for char in raw) and not any(char.isdigit() for char in raw)
