...
def build_pick_sheets(consolidated: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    special_locations = df["Exception Reason"] == "Special non-numeric location"
    df.loc[special_locations, "_Route Aisle"] = -1
    df.loc[special_locations, "_Route Bay"] = -1
    df.loc[special_locations, "_Route Vertical"] = ""
    df.loc[special_locations, "_Route Lateral Number"] = -1
    df.loc[special_locations, "_Route Lateral"] = df.loc[special_locations, "From Bin"].fillna("").astype(str)
